...
for interval_index in range(3):
    ...
    X_hex = X + 0.5 * Y
    Y_hex = (np.sqrt(3)/2) * Y
    alpha = interval[interval_index] # interval[0]
    Y_hex = alpha * Y_hex
    ...
    # def update(step):
        ...
        crop_y = []
        crop_values = []
        # for select_x in X_flat:
        #     for select_y in Y_flat:
        #         if select_x > 25 and select_x < 60 and select_y > 10 and select_y < 45:
        #             crop_x.append(select_x)
        #             crop_y.append(select_y)
        #             # crop_values.append(values[i])
        #             print(crop_x)
        #             # print(values[i])
        # # crop_x = [select_x for select_x in X_flat if select_x > 25 and select_x < 60]
        # # crop_y = [select_y for select_y in Y_flat if select_y > 10 and select_y < 45]
        # # crop_values = [values[i] for i in range(len(values)) if X_flat[i] > 25 and X_flat[i] < 60 and Y_flat[i] > 10 and Y_flat[i] < 45]
        # crop_indices = [i for i in range(len(X_flat)) if 25 < X_flat[i] < 60 and 10 < Y_flat[i] < 45] # grid_size
        # crop_indices = [i for i in range(len(X_flat)) if 0 < X_flat[i] < 100 and 00 < Y_flat[i] < 80]
        crop_indices = [i for i in range(len(X_flat)) if int(grid_size*0.4) < X_flat[i] < int(grid_size*0.9) and int(grid_size*0.2) < Y_flat[i] < int(grid_size*0.7)]
        crop_x = [X_flat[i] for i in crop_indices]
        crop_y = [Y_flat[i] for i in crop_indices]
        crop_values = [values[i] for i in crop_indices]
        # if step % 1000 == 0:
        # print(step)
        color_map = ['red' if v == 0 else 'blue' for v in values]
        crop_color_map = ['red' if v == 0 else 'blue' for v in crop_values]

        '''# plt.clf()
        ax.clear()
        # plt.scatter(X_flat, Y_flat, c=color_map, marker='H', linewidth=3)
        ax.scatter(crop_x, crop_y, c=crop_color_map, marker='H', linewidth=3)
        ...
        plt.pause(0.01)'''
            ...
            # def update(step):
            #     ax.clear()
            #     crop_indices = [i for i in range(len(X_flat)) if 25 < X_flat[i] < 60 and 10 < Y_flat[i] < 45]
            #     crop_x = [X_flat[i] for i in crop_indices]
            #     crop_y = [Y_flat[i] for i in crop_indices]
            #     crop_values = [values[i] for i in crop_indices]
            #     crop_color_map = ['red' if v == 0 else 'blue' for v in crop_values]
            #     ax.scatter(crop_x, crop_y, c=crop_color_map, marker='H', linewidth=3)
            #     ax.set_title('Coordinates with Assigned Values (0 or 1)')
            #     ax.set_xlabel('X')
            #     ax.set_ylabel('Y')
    ...
    # ax.scatter(crop_x, crop_y, c=crop_color_map, marker='H', linewidth=3)
    selected_crop_x = [crop_x[i] for i in range(len(crop_values)) if crop_values[i] == 1]
    selected_crop_y = [crop_y[i] for i in range(len(crop_values)) if crop_values[i] == 1]
    print("Number of 1s in the cropped region: ", len(selected_crop_x))
    print("Number of 0s in the cropped region: ", len(crop_x) - len(selected_crop_x))

    # print('selected_crop_x:', selected_crop_x)
    # print('selected_crop_y:', selected_crop_y)
    

    save_data = []
    for i in range(len(selected_crop_x)):
        save_data.append([selected_crop_x[i], selected_crop_y[i]])
    np.savetxt(f'/Users/user1/Documents/GitHub/turing-patterns/20241212_grid_data_{grid_size}_{steps}_step_c=0_25_alpha=_{alpha}_value1_coordinates.txt', save_data)
    # fig2, ax2 = plt.subplots(figsize=(6,6))
    # ax2.scatter(selected_crop_x, selected_crop_y, c='blue', marker='H', linewidth=3)
    # plt.show()
# ani = animation.FuncAnimation(fig, update, frames=np.arange(100), interval=100)
# ani.save('/Users/user1/Documents/GitHub/turing-patterns/animation.gif', writer='imagemagick')
