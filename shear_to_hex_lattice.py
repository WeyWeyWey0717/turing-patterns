...
values = np.fliplr(np.loadtxt('./2dRD_500_20000_third_A_only.txt', dtype=float)[0:100,0:100])
...
plt.scatter(xx, yy, c=values, cmap='viridis', label='Original', marker='o')
plt.xlabel('X')
plt.ylabel('Y')
...
plt.scatter(transformed_coordinates[:, :, 0], transformed_coordinates[:, :, 1], c=values, cmap='viridis', label='Transformed', marker='o')
plt.xlabel('X')
plt.ylabel('Y')
plt.colorbar(label='Value')
plt.grid(True)
